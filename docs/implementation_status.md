...
49. Geomagic AutoSurface 下一步路线已根据 `scripts/industrial_sew.py` 研究结果确定：先做 T6.5.1 Apply failure diagnostics，把 repair 前后 face/edge/shell/solid、Gate before/after stats、STEP roundtrip stats、BRepCheck/free/multiple edge 等关键字段写入 `PatchReplacementReport` 并显示到 GUI，必要时导出 rejected after shape 作为 debug artifact；再做 T6.6 Industrial Adaptive Sewing，将老师脚本的三阶段策略迁移到 C++ repair pipeline：`ShapeFix_Shape` / face orientation 修复、`ShapeUpgrade_UnifySameDomain`、多 tolerance `BRepBuilderAPI_Sewing` loop、`ShapeFix_Shell`、`BRepBuilderAPI_MakeSolid`、`ShapeFix_Solid`、collapsed guard 和 best-result selection。该路线不放宽 StrictTopologyGate，不绕过 rollback，不把 STL crop boundary 或 Geomagic patch outer boundary 当最终 CAD boundary，multi-face patch 仍是主路径，redo 仍只复用缓存 afterDocument。
```
...
| T6.5.1 / T6.6 后续测试计划 | 待实现 | T6.5.1 覆盖 ApplyFailed GUI/report 诊断字段、Gate stats、roundtrip stats、debug rejected after artifact；T6.6 覆盖 adaptive sewing 多 tolerance 尝试、preferred tolerance、collapsed guard、shell-to-solid、multi-face 不 unsupported、GateFailed rollback、redo 不重跑 sewing |
